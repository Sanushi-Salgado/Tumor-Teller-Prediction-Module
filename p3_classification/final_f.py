...
def create_dataset(data_frame):
    # Drop the class column since it is useless
    # data_frame.drop('class', axis=1, inplace=True)

    data_frame.groupby(TOP_LEVEL_TARGET).size()

    # # Separate input feature & target variable
    X = data_frame.drop(TOP_LEVEL_TARGET, axis=1)
    y = data_frame[TOP_LEVEL_TARGET]
    ...
    train_class_labels = y_resampled['region']
    region_dataset = X_resampled.join(train_class_labels)
    ...
    # dataset = data_frame[data_frame[TOP_LEVEL_TARGET].isin(TOP_LEVEL_CLASSES)]
    # print(dataset.shape)
    # print(dataset.groupby(TOP_LEVEL_TARGET).size())
    region_dataset.to_csv('../resources/datasets/top_level_dataset.csv', index=False)

    region_dataset = pd.read_csv('../resources/datasets/top_level_dataset.csv')
    print(region_dataset.groupby(TOP_LEVEL_TARGET).size())
...
def upper_region_classifier():
        ...
        joblib.dump(imba_pipeline, filename='../resources/models/ur_classifier.pkl')
...
def ip_region_classifier():
    ...
    # Separate training feature & training labels
    X = ip_dataset.drop(['class', 'small-intestine'], axis=1)
    y = ip_dataset['class']

    # Separate testing feature & testing labels
    X_test = ip_dataset.drop(['class', 'small-intestine'], axis=1)
    y_test = ip_dataset['class']
...
def classify_by_region_cv():
    data_frame = pd.read_csv('../resources/datasets/top_level_dataset.csv')
    ...
    joblib.dump(final_model, '../resources/models/FinalParentModel.pckl')
...
create_dataset(pre_processed_data)

classify_by_region_cv()
